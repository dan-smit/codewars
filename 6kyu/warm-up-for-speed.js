...
function sortedCommByDigs(a1, a2) {
    const set = new Set(a1);
    const commons = a2.reduce((acc, el) => {
      return (set.has(el) && acc.push(el), acc);
    }, []);
    const f = (() => {
      const dr = n => [...(n + '')].reduce((a, b) => a + +b, 0);
      const dsddr = n => [...(dr(n) + '')].reduce((a, b) => a + (+b) * (+b), 0);
      return (n) => dr(n) + dsddr(n);
    })();
    return [...new Set(commons)].sort((a, b) => f(b) - f(a) || a - b);
  }